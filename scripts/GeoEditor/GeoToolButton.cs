...
public partial class GeoToolButton : TextureButton
{
	...
	public override void _EnterTree()
	{
		...
		GeoEditor.GeometryEditor.ChangedTool += ToolChanged;
	}
	...
	private void PressedButton()
	{
		...
		{
			if (GeoEditor.GeometryEditor.CurrentSecondaryTool == ToolNumber) return;
			GeoEditor.GeometryEditor.CurrentTool = ToolNumber;
			GetNode<Label>("%ToolLabel").Text = "Primary Tool: " + _toolNames[ToolNumber];
		}
		else if (Input.IsActionJustReleased("mouse_right"))
		{
			if (GeoEditor.GeometryEditor.CurrentTool == ToolNumber) return;
			GeoEditor.GeometryEditor.CurrentSecondaryTool = ToolNumber;
			GetNode<Label>("%SecondaryToolLabel").Text = "Secondary Tool: " + _toolNames[ToolNumber];
		}
		
		GeoEditor.GeometryEditor.ToolChanged();
	}

	private void ToolChanged(object sender, EventArgs args)
	{
		if (GeoEditor.GeometryEditor.CurrentTool == ToolNumber) SelfModulate = Colors.Red;
		else if (GeoEditor.GeometryEditor.CurrentSecondaryTool == ToolNumber) SelfModulate = Colors.Blue;
