...
public partial class TileEditor : Editor
{
    ...
    public static Tile CurrentTile
    {
        get
        {
            if (!Tiles.ContainsKey(_currentTile))
                return new Tile();
    ...
    public static readonly TileLayer[] Layers =
    {
        new(0),
        new(1),
        new(2)
    ...
    private static int _currPalette;
    private static List<Texture2D> _palettes;

    public static string[,,] Data;
    
    public static bool RenderMode;
    ...
    public override async void _Ready()
    {
        ...
        {
            Debug.Write(Utils.WARNING_STR + "Tile directory not set, skipping tile editor setup.");
            return;
        }
    ...
    public override void _Process(double delta)
    {
        ...
        if (Input.IsActionPressed("mouse_left") && !DraggableControl.IsDragging && !DraggableControl.OverControl)
        {
            ...
            Layers[Petrichor.CurrentLayer].SetTileMesh(tileTex, ref currTile, cellPos.X, cellPos.Y);
    ...
    {
        if (Petrichor.CurrentEditor != this)
            return;
    ...
    {
        _palettes = Utils.LoadImageFolder("res://palettes/", ResourceLoader.CacheMode.Ignore);
        ChangePalette(0);
        Debug.WriteLine(Utils.INFO_STR + "Palettes reloaded.");
    }
